...
PLATFORM_PERMISSION_CODE_RE = re.compile(r'^platform\.[a-z_]+\.[a-z_]+$')


class PlatformPermissionSerializer(serializers.ModelSerializer):
    class Meta:
        model = PlatformPermission
        fields = ('id', 'code_name', 'name', 'module', 'description', 'is_builtin')
        read_only_fields = ('is_builtin',)

    def validate_code_name(self, value):
        if not PLATFORM_PERMISSION_CODE_RE.match(value or ''):
            raise serializers.ValidationError(
                'code_name must match platform.<module>.<action> in snake_case.'
            )
        return value
